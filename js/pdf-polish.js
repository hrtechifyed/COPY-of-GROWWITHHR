/* ==========================================================
   ...
   Compact A4 renderer with selectable light and dark HRTechify themes.
   ========================================================== */
(function installGrowWithHRPdfPolish(window, document) {
    ...
    function resolveTheme(payload = {}) {
        ...
        const requested = cleanText(
            payload.theme || payload.reportTheme || payload.pdfTheme || selected?.value || stored,
            "light"
        );
    ...
    function createWriter(doc, model, logoDataUrl, options) {
        ...
        function header() {
            ...
            setFont("normal", 7.8);
            setText(theme.muted);
            doc.text(options.runningTitle, PAGE.width - PAGE.right, PAGE.headerY, { align: "right" });
            setDraw(theme.line);
            doc.setLineWidth(0.3);
            doc.line(PAGE.left, 16, PAGE.width - PAGE.right, 16);
        }
        ...
        function cover() {
            ...
            if (logoDataUrl) {
                try { doc.addImage(logoDataUrl, "PNG", PAGE.left, 15, 30, 30, undefined, "FAST"); } catch (_error) {}
            }
            setFont("bold", 13);
            setText(theme.white);
            doc.text("HRTechify", logoDataUrl ? 51 : PAGE.left, 27);
            setFont("normal", 8.8);
            setText([198, 211, 230]);
            doc.text("People • Technology • Growth", logoDataUrl ? 51 : PAGE.left, 34);

            setFont("bold", 9.2);
            setText(theme.accent);
            doc.text(options.coverLabel, PAGE.left, 70);
            setFont("bold", 27);
            setText(theme.white);
            const title = split(options.coverTitle, 170);
            doc.text(title, PAGE.left, 88, { lineHeightFactor: 1.08 });
            const titleY = 88 + title.length * lineHeight(27, 1.08) + 7;
            setFont("normal", 15);
            setText([218, 228, 242]);
            doc.text(split(model.companyName, 170), PAGE.left, titleY, { lineHeightFactor: 1.18 });

            setFill([15, 31, 54]);
            doc.roundedRect(PAGE.left, 170, usableWidth, 53, 4, 4, "F");
            setDraw(theme.accent);
            doc.setLineWidth(0.8);
            doc.line(PAGE.left, 170, PAGE.left, 223);
            setFont("normal", 10.2);
            setText([229, 236, 246]);
            doc.text(split(options.coverIntro, usableWidth - 18), PAGE.left + 9, 187, { lineHeightFactor: 1.45 });

            setFont("normal", 9);
            setText([176, 190, 210]);
            doc.text(`Prepared for ${model.recipientName}`, PAGE.left, 238);
            doc.text(`Prepared ${formatDate(model.generatedAt)}`, PAGE.left, 246);
            doc.text(`Report style: ${options.theme === "dark" ? "Dark presentation" : "Light print-friendly"}`, PAGE.left, 254);

            setFont("italic", 8.2);
            doc.text(
                split(options.isSample
                    ? "Illustrative sample using fictional organisation information."
                    : "Confidential working document. Verify legal and regulatory requirements with qualified professionals and current official sources.", usableWidth),
                PAGE.left,
                274,
        ...
        function sectionHeading(overline, title, introduction = "") {
            ensureSpace(introduction ? 31 : 23);
            setFont("bold", 7.8);
            setText(theme.accentDark);
            ...
            doc.text(lines, PAGE.left, cursorY, { lineHeightFactor: 1.1 });
            cursorY += lines.length * lineHeight(16, 1.1) + 3;
            setDraw(theme.accent);
            doc.setLineWidth(0.7);
            doc.line(PAGE.left, cursorY, PAGE.left + 24, cursorY);
            cursorY += 6;
            if (introduction) paragraph(introduction, { size: 9.3, colour: theme.muted, spacingAfter: 5 });
        }

        function paragraph(text, settings = {}) {
            const size = settings.size || 9.5;
            const factor = settings.lineHeight || 1.38;
            const spacingAfter = settings.spacingAfter ?? 4;
        ...
        function subheading(title) {
            ...
            setText(theme.heading);
            const lines = split(title, usableWidth);
            doc.text(lines, PAGE.left, cursorY, { lineHeightFactor: 1.16 });
            cursorY += lines.length * lineHeight(11.2, 1.16) + 3;
        }

        function numberedList(items, settings = {}) {
            unique(toArray(items)).forEach((item, index) => {
                const lines = split(item, usableWidth - 14);
                const height = lines.length * lineHeight(9.2, 1.36) + 3;
                ensureSpace(Math.min(height, 20));
                setFill(theme.accentSoft);
                doc.circle(PAGE.left + 3.5, cursorY - 1.5, 3.1, "F");
                setFont("bold", 7.8);
                setText(theme.accentDark);
                doc.text(String(index + 1), PAGE.left + 3.5, cursorY - 0.4, { align: "center" });
                setFont("normal", 9.2);
                setText(theme.text);
                doc.text(lines, PAGE.left + 10, cursorY, { lineHeightFactor: 1.36 });
                cursorY += height;
            });
            cursorY += settings.spacingAfter ?? 1;
        }

        function bulletList(items) {
            unique(toArray(items)).forEach((item) => {
                const lines = split(item, usableWidth - 10);
                const height = lines.length * lineHeight(9.2, 1.36) + 2.5;
                ensureSpace(Math.min(height, 20));
                setFill(theme.accent);
                doc.circle(PAGE.left + 2, cursorY - 1, 0.9, "F");
                setFont("normal", 9.2);
                setText(theme.text);
                doc.text(lines, PAGE.left + 7, cursorY, { lineHeightFactor: 1.36 });
                cursorY += height;
            });
            cursorY += 1;
        }
        ...
        function profileRow(label, value, alternate) {
            ...
            const textHeight = Math.max(
                labelLines.length * lineHeight(8.7, 1.3),
                valueLines.length * lineHeight(9.3, 1.34)
            );
            const rowHeight = Math.max(12, textHeight + 7);
            ...
            setFont("bold", 8.7);
            setText(theme.heading);
            doc.text(labelLines, PAGE.left + 6, cursorY + 1, { lineHeightFactor: 1.3 });
            setFont("normal", 9.3);
            setText(theme.text);
            doc.text(valueLines, PAGE.left + labelWidth + 4, cursorY + 1, { lineHeightFactor: 1.34 });
        ...
        function recommendationCard(item, index) {
            ...
            const estimated = 22 + split(title, usableWidth - 16).length * 5 +
                split(recommendation, usableWidth - 16).length * 4 + howTo.length * 8;
            ensureSpace(Math.min(estimated, 75));

            setFill(theme.panel);
            setDraw(theme.line);
            const startY = cursorY - 4;
            doc.roundedRect(PAGE.left, startY, usableWidth, 10, 3, 3, "FD");
            setFont("bold", 7.6);
            setText(theme.accentDark);
            doc.text(`RECOMMENDATION ${index + 1}`, PAGE.left + 7, cursorY + 2);
            cursorY += 9;
            subheading(title);
            if (observation) paragraph(observation, { size: 8.9, colour: theme.muted, spacingAfter: 3 });
            paragraph(recommendation, { size: 9.4, spacingAfter: 4 });
            if (howTo.length) {
                setFont("bold", 8.1);
                setText(theme.accentDark);
                doc.text("HOW TO IMPLEMENT", PAGE.left, cursorY);
                cursorY += 5;
                numberedList(howTo, { spacingAfter: 2 });
            }
            const meta = [item.owner ? `Owner: ${item.owner}` : "", item.timeframe ? `Timing: ${item.timeframe}` : ""].filter(Boolean);
            if (meta.length) paragraph(meta.join("   •   "), { size: 8.2, colour: theme.muted, spacingAfter: 2 });
            if (item.resourceUrl) {
                ensureSpace(8);
                setFont("bold", 8.7);
                setText(theme.accentDark);
                const label = cleanText(item.resourceLabel, "Open implementation resource");
                doc.textWithLink(`${label} →`, PAGE.left, cursorY, {
                    url: new URL(item.resourceUrl, window.location.href).href
                });
                cursorY += 8;
            }
        }
        ...
        function footers() {
            ...
            for (let page = 1; page <= total; page += 1) {
                ...
                if (page > 1) {
                    setDraw(theme.line);
                    doc.setLineWidth(0.3);
                    doc.line(PAGE.left, 281, PAGE.width - PAGE.right, 281);
                }
                setFont("bold", 7.2);
                setText(page === 1 ? [190, 203, 222] : theme.accentDark);
                doc.text("HRTechify | GrowWithHR", PAGE.left, PAGE.footerY);
                setFont("normal", 7);
                setText(page === 1 ? [170, 185, 205] : theme.muted);
                doc.text(`Page ${page} of ${total}`, PAGE.width - PAGE.right, PAGE.footerY, { align: "right" });
            }
        }

        paintPage();
        return {
            cover, addPage, ensureSpace, sectionHeading, paragraph, subheading,
            numberedList, bulletList, profileRow, profileTable, recommendationCard, footers
        };
    }
    ...
    function render(doc, model, logoDataUrl, options) {
        ...
        writer.sectionHeading("Recommended actions", "Strategic Recommendations", "Each recommendation includes practical implementation steps and a reusable starting resource.");
        (model.recommendations || []).forEach((item, index) => writer.recommendationCard(item, index));
        ...
        writer.sectionHeading("Compliance review", "What You Should Review", "Governance prompts only; confirm applicability with qualified advisers and official sources.");
        ...
        writer.paragraph(options.isSample
    ...
    async function buildAdvisoryPdf(payload = {}) {
        ...
        const options = {
            ...
            coverLabel: cleanText(payload.coverLabel, payload.isSample ? "ILLUSTRATIVE SAMPLE EXECUTIVE ADVISORY" : "PERSONALISED EXECUTIVE ADVISORY"),
            coverTitle: cleanText(payload.coverTitle, "Executive Advisory"),
            coverIntro: cleanText(payload.coverIntro, "A practical leadership briefing connecting your organisation profile with people, governance, compliance and growth priorities.")
        };
    ...
    async function downloadAdvisoryPdf(payload = {}) {
        ...
        doc.save(filename);
        try { window.localStorage?.setItem(LAST_DOWNLOAD_KEY, new Date().toISOString()); } catch (_error) {}
        return result;
    }
    ...
    async function generatePDFReport() {
        ...
        try {
            const result = await buildAdvisoryPdf({
                ...
                theme: resolveTheme(),
                lead: { email: cleanText(document.getElementById("userEmail")?.value), deliveryRequested: true }
            });
    ...
    window.dispatchEvent(new CustomEvent("growwithhr:pdf-polish-ready", { detail: { version: VERSION } }));
})(window, document);
