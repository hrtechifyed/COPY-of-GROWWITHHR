...
(function initialiseGmailService(window) {
    ...
    function getEndpoint() {
        const explicitEndpoint = document.body?.dataset?.emailEndpoint ||
    ...
    function getHealthEndpoint() {
        const explicitHealthEndpoint = document.body?.dataset?.healthEndpoint ||
    ...
    function removeDataUriPrefix(value) {
        ...
        return source.startsWith("data:") && commaIndex >= 0
    ...
    async function postDelivery(action, payload = {}) {
        ...
        try {
            ...
            if (result.customerSent !== true && result.customerStatus !== "sent") {
                throw new Error(
                    result.error || "The email server did not confirm delivery."
                );
            }
            return saveStatus({
                ok: true,
    ...
    function operatingStatesFromAnswers(answers = {}) {
        ...
        if (indexed.length) {
            return rawOperatingStateList(indexed).slice(0, MAX_OPERATING_STATES);
        }

        const primaryState = cleanText(answers.primaryState);
        if (
            !primaryState ||
            primaryState.toLowerCase() === PAN_INDIA_LABEL.toLowerCase()
        ) {
            return [];
        }
    ...
    function resolvedOperatingStateCount(answers = {}) {
        ...
        if ([PAN_INDIA_VALUE, "1", "2", "3"].includes(configured)) {
            return configured;
        }
        const primaryState = cleanText(answers.primaryState);
        ...
        return String(
            Math.min(MAX_OPERATING_STATES, Math.max(1, stateCount || 1))
        );
    }
    ...
    function renderOperatingFootprint(context = {}) {
        ...
        const fields = context.fields ||
            window.GrowWithHRModules?.AssessmentFields?.create?.({ answers });
        if (!fields) return "";
            ...
            return `
                <div data-operating-state-slot="${stateNumber}" ${visible ? "" : "hidden"}>
                    ${fields.datalistField({
                        ...
                        options: Array.isArray(definitions.STATES)
                            ? definitions.STATES
                            : [],
                        required: visible,
                        value
    ...
    function addTemplateSampleData(value, industry) {
        const sampleData = [
            {
                ...
                location: "Bengaluru",
                status: "Active",
                note: `${industry || "Industry"} sample record`
            },
            {
                id: "EMP-002",
        ...
        if (Array.isArray(report.recommendations)) {
            report.recommendations = report.recommendations.map((item) => ({
                ...item,
                templateSampleData:
                    Array.isArray(item?.templateSampleData) &&
                    item.templateSampleData.length >= 5
                        ? item.templateSampleData
                        : sampleData.map((record) => ({ ...record }))
            }));
        }
        report.templateSampleData =
            Array.isArray(report.templateSampleData) &&
            report.templateSampleData.length >= 5
                ? report.templateSampleData
                : sampleData;
    ...
    wrapModule("AssessmentValidation", (original) => {
        ...
        function validateOperatingFootprint(answers = {}) {
            const result = original.createResult(answers);
            const normalized = normaliseOperatingFootprintAnswers(
            ...
            if (![PAN_INDIA_VALUE, "1", "2", "3"].includes(count)) {
                result.valid = false;
                result.errors.operatingStateCount =
                    "Choose how many states or union territories you operate from.";
                result.firstInvalidField =
                    result.firstInvalidField || "operatingStateCount";
                ...
                states.forEach((state, index) => {
                    const field = `operatingState${index + 1}`;
                    if (!state) {
                        result.valid = false;
                        result.errors[field] =
                            `Enter state or union territory ${index + 1}.`;
                        result.firstInvalidField =
                            result.firstInvalidField || field;
                    } else if (
                        allowedStates.size &&
                        !allowedStates.has(state.toLowerCase())
                    ) {
                        result.valid = false;
                        result.errors[field] =
                            "Choose a state or union territory from the suggestions.";
                        result.firstInvalidField =
                            result.firstInvalidField || field;
                    }
                });
                ...
                if (
                    states.filter(Boolean).length === requiredCount &&
                    uniqueStates.length !== requiredCount
                ) {
                    result.valid = false;
                    result.errors.operatingStates =
                        "Enter a different state or union territory in each box.";
                    result.firstInvalidField =
                        result.firstInvalidField || "operatingState1";
                }
                result.normalizedAnswers.operatingStates = uniqueStates;
                result.normalizedAnswers.primaryState = uniqueStates.join("; ");
                result.normalizedAnswers.stateCoverage =
                    result.normalizedAnswers.primaryState;
            }
            ...
            result.normalizedAnswers.indiaComplianceScopeNotice =
        ...
        return Object.freeze({
            ...
            validateLead(lead) {
                ...
                const emailValidation = validateRecipientEmails(
                ...
                if (!emailValidation.valid) {
                    ...
                    result.firstInvalidField =
                        result.firstInvalidField || "email";
                } else {
    ...
    wrapModule("ReportMapper", (original) => {
        ...
        const wrapResult = (result, payload) => {
            ...
            if (!report || typeof report !== "object") return report;
            const footprint = normaliseOperatingFootprintAnswers(
                payload?.answers || {}
            );
            return {
                ...report,
                primaryState: cleanText(
                    report.primaryState || footprint.primaryState
                ),
            ...
            wrapped[method] = function requestedChangesMapper(payload) {
                ...
                if (
                    ...
                    return {
                        ...result,
                        report: wrapResult(result.report, prepared),
                        answers: normaliseOperatingFootprintAnswers(
                            result.answers || prepared.answers
                        )
                    };
                }
    ...
    function synchroniseOperatingStateFields() {
        ...
        const checked = fieldset.querySelector(
            'input[name="operatingStateCount"]:checked'
        );
        ...
        const numericCount = panIndia
            ? 0
            : Math.min(
                MAX_OPERATING_STATES,
        ...
        for (let index = 1; index <= MAX_OPERATING_STATES; index += 1) {
            const slot = document.querySelector(
                `[data-operating-state-slot="${index}"]`
            );
            const separator = document.querySelector(
                `[data-operating-state-separator="${index}"]`
            );
            const input = document.getElementById(`operatingState${index}`);
            ...
            if (input) {
                ...
                if (visible && cleanText(input.value)) {
                    states.push(cleanText(input.value));
                }
            }
            if (separator) {
                separator.hidden = !(visible && index < numericCount);
            }
        }
        ...
        setAssessmentAnswer(
            "primaryState",
            panIndia ? PAN_INDIA_LABEL : uniqueStates.join("; ")
        );
        setAssessmentAnswer(
            "stateCoverage",
            panIndia ? PAN_INDIA_LABEL : uniqueStates.join("; ")
    ...
    document.addEventListener("change", (event) => {
        ...
        if (
            ...
            (
                ...
                /^operatingState[1-3]$/.test(target.name)
            )
        ) {
    ...
    function configureEmailInput() {
        ...
        if (emailInput && !emailInput.dataset.multiEmailConfigured) {
            ...
            emailInput.placeholder =
                "name@company.com; colleague@company.com";
        ...
        if (emailHelp) {
            emailHelp.textContent =
                "Enter up to five email addresses separated by semicolons (;). Each address will receive the advisory.";
        }
    }
    ...
    function configureAssessmentEnhancements() {
        ...
        if (
            storyContainer &&
            !storyContainer.dataset.footprintObserverConfigured
        ) {
