...
(function initialiseGmailService(window) {
    ...
    function addAllPrioritiesOption(markup) {
        ...
        if (!source.includes('data-field-wrapper="priorities"') || source.includes(`value="${ALL_PRIORITIES_VALUE}"`)) return source;
        const option = `
        ...
        return source.replace(/(<p\s+id="prioritiesError")/, `${option}$1`)
            .replace("Choose up to three priorities.", "Choose up to three priorities, or select All of the above.");
    }
    ...
    wrapModule("AssessmentScreens", (original) => {
        ...
        return Object.freeze({
            ...original,
            renderPeopleReadiness(context) { return addAllPrioritiesOption(original.renderPeopleReadiness(context)); },
            renderMoment(identifier, context) { return addAllPrioritiesOption(original.renderMoment(identifier, context)); },
    ...
    wrapModule("ReportMapper", (original) => {
        ...
        const preparePayload = (payload = {}) => ({
            ...payload,
            answers: { ...(payload.answers || {}), priorities: expandAllPriorities(payload.answers?.priorities) }
        });
        const wrapResult = (result, payload) => addTemplateSampleData(result, payload?.answers?.industry || payload?.report?.industry);
    ...
    document.addEventListener("DOMContentLoaded", configureEmailInput);
    if (document.readyState !== "loading") configureEmailInput();
})(window);
